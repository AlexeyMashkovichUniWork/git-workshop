...
int read_interval(struct interval_t *interval) {
	if (printf("Enter interval's left border: ") < 0) {
		ERROR_PRINT("Cannot wtite to stdout to stdout");
		return -1;
	}
	if (scanf("%lf", &(interval->left_border)) != 1) {
		ERROR_PRINT("Cannot read interval's left border");
	}
	if (interval->left_border < 0) {
		ERROR_PRINT("Left border of the interval must be greater than or equal to 0");
		return -1;
	}
	if (printf("Enter interval's right border: ") < 0) {
		ERROR_PRINT("Cannot wtite to stdout");
		return -1;
	}
	if (scanf("%lf", &(interval->right_border)) != 1) {
		ERROR_PRINT("Cannot read interval's right border");
	}
	if (interval->right_border > M_PI) {
		ERROR_PRINT("Right border of the interval must be less than or equal to pi");
		return -1;
	}
	if (interval->left_border > interval->right_border) {
		ERROR_PRINT("Right border of the interval must be greater than left");
		return -1;
	}
	return 0;
}
